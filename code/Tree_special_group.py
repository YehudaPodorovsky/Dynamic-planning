...
def dfs(node):
    ...
    for child in node.children:
        dfs(child)
        node.values['plus'] += child.values['OPT']
        node.values['minus'] += child.values['plus']

    node.values['OPT'] = min(node.values['plus'], node.values['minus'])
    print(node.values)
...
dfs(nodes[0])
print()
print("Min special group in Tree is", nodes[0].values['OPT'])
